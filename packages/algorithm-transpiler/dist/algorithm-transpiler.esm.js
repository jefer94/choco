...
function vars (code, store) {
  var _compose$split = compose(comments, spaces, ignoreSentences)(code).split('\n'),
...
function transform (code) {
  ...
  var line = compose(stripCode, comments)(code); // stripCode(code)
  ...
  Object.keys(line).map(Number).forEach(function (i) {
    ...
    if (line[i] === '') return;
    line = ifIsEqual(line);
    line[i] = compose(forLoopCondition, doWhileLoopCondition)(line[i]); // each word is separated into a array
